...
import { LINKS } from '@/constrant/data'
...
const Nav = () => {
  ...
  return (
    <nav className='fixed max-lg:bottom-2 max-lg:w-full flexCenter z-50 lg:right-2 lg:h-full '>
      ...
      <div className='flex gap-x-6 lg:gap-x-10 px-4 py-3 rounded-full bg-tertiary/10 lg:flex-col  lg:gap-y-10  '>

        {LINKS.map((link) => (
          <Link key={link.title} href={link.path} className={`${link.path === pathname ? "text-white" : "text-tertiary"} relative group`}>
            <motion.span
              className=' h-8 w-8 bg-primary flexCenter rounded-full bold-18 z-50'
              animate={
                {
                  backgroundColor: ['#ffda79', '#ffabe7', '#d2bcf3', '#edf3d8', '#ffda79'],
